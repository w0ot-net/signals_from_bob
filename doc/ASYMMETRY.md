...
- If a retransmit would exceed the per-request response cap, Bob responds with
  KEEPALIVE + POLL_HINT only when the response cap can carry at least one
  segment byte (MIN_PACKET_MTU); otherwise he sends KEEPALIVE without POLL_HINT
  and leaves the packet pending for a larger-cap poll
- Retransmits reuse existing sequence numbers; outstanding count stays capped
...
keepalive response with no segments, Alice returns to the idle polling
interval.
