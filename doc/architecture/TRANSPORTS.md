...
DNS TXT note: response MTU is derived from `dns_edns_size` and clamped to a
fixed response cap computed from the base domain/label limits (so TXT answers
fit within the packet limit). `dns_txt_response_cap` can optionally lower the
response packet MTU further.
...
| DNS TXT | Query MTU from `base_domain` and `label_max_len`; response MTU from `dns_edns_size` (TXT RDATA size), clamped to a fixed cap derived from the max query name; optional `dns_txt_response_cap` can lower further | TXT response size must fit within the EDNS payload |
