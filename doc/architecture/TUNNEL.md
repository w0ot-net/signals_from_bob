...
packet when idle and with queued data when available. Keepalive is only used
when no segments are sent; if any channel has pending data, the response
carries `HAS_SEGMENTS`. If either side has actual data to send, the packet
...
idle. Keepalive packets should not carry segments.
