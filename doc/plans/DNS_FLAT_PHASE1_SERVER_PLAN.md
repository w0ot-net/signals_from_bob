...
## Summary
Add server-side support for `--sfb-flat`, package `sfb_flat.py` into gzipped
chunks, and serve stager metadata/chunks via CNAME responses in DNS.
...
## Plan
1. Add a server-only `--sfb-flat <path>` CLI option in `sfb/cli.py`.
...
3. Add stager query handling in `sfb/transport/dns/dns_server.py`.
...
4. Add minimal logging for stager paths.
