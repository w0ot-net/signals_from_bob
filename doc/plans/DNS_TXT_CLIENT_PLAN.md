...
1. Copy DNS utility helpers.
   - Copy `sfb/transport/dns/dns_utils.py` verbatim to
     ...
     `sfb.transport.dns`.
2. Configuration and resolver selection.
   - Validate `Config` instance and capture DNS fields needed by the client.
   - Parse `dns_resolver` if provided; otherwise load system resolvers via the
     local `dns_txt.dns_utils` copy.
   - Log client configuration with `dns_txt.client_config`.
3. Socket and EDNS0 setup.
...
5. Pending tracking and send path.
