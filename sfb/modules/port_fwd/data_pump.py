# -*- coding: ascii -*-
"""
Shared port forward data pump helpers.

Each port forward connection uses two threads; each pump handles one socket/channel.
"""
...
def _pump_poll_bounds(config):
    base = config.port_fwd_pump_poll_timeout
    if base is None or base <= 0:
        base = 0.01
    max_wait = config.port_fwd_pump_backoff_max
...
def _outbound_cap(config):
    ...
    if cap <= 0:
        cap = config.port_fwd_relay_buffer_size
    return cap
        ...
        while not stop_event.is_set():
            ...
            while rlist and not stop_event.is_set():
                ...
                if available <= 0:
                    buffer_full_count += 1
                    break
                read_size = config.port_fwd_relay_buffer_size
                    ...
                    if eof_callback is not None:
                        if not stop_event.is_set():
                            log_event(
                                logger,
                                logging.DEBUG,
                                'fwd.relay_half_close_request',
                                'Port forward relay half-close requested',
                                lambda: add_fields(fwd_fields(
                                    rid=rid,
                                    ch=ch,
                                    side=side,
...
def pump_channel_to_socket(channel, sock, config, logger, stop_event,
    ...
    try:
        ...
        base_backoff, max_backoff = _pump_poll_bounds(config)
        backoff = base_backoff
        write_timeout = config.port_fwd_relay_write_timeout
        last_send = None
                ...
                if write_timeout is not None and now - last_send > write_timeout:
                    ...
                    if not stop_event.is_set():
                        log_event(
                            ...
                            logging.DEBUG,
                            'fwd.pump_timeout',
                            'Port forward pump socket send timeout',
                            lambda: add_fields(fwd_fields(
                                rid=rid,
                                ch=ch,
                                side=side,
            ...
            if not channel_closed and outbound_size < outbound_limit:
                ...
                if read_size > 0:
                    if outbound_size:
                        ...
                        read_timeout = min(config.port_fwd_relay_channel_timeout, backoff)
                    try:
            ...
            if channel_closed and outbound_size == 0:
                if shutdown_pending:
                    log_event(
                        logger,
                        logging.DEBUG,
                        'fwd.relay_shutdown_write',
                        'Port forward relay socket write shutdown',
                        lambda: add_fields(fwd_fields(
                            rid=rid,
                            ch=ch,
                            side=side,
