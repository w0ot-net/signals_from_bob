...
from .tls_handshake_bump_helpers import (
    _IN_PROGRESS,
    _TEMP_ERRORS,
    _SOFT_CONNECT_ERRORS,
    _RESET_ERRORS,
    _build_connect_request,
    _build_https_request_parts,
    _build_https_request,
    _create_ssl_context,
    _wrap_socket,
    _ssl_wants_read,
    _ssl_wants_write,
...
class TlsHandshakeBumpClient(Transport):
    ...
    def recv(self, timeout=None):
        ...
        while True:
            ...
            for sock in ready_w:
                result = self._handle_writable(sock, now)
                if result is not None:
                    return result

            for sock in ready_r:
                result = self._handle_readable(sock, now)
                if result is not None:
                    return result
    ...
    def _handle_writable(self, sock, now):
        ...
        if state.ssl_want == 'write':
            result = self._recv_response(corr_id, state)
            if result is not None:
                return result

    def _handle_readable(self, sock, now):
        corr_id, state = self._lookup_state(sock)
        if state is None:
            return None
        if state.connecting:
    ...
    def _build_select_lists(self):
        ...
        for state in self._pending_state.values():
            ...
            if state.connecting:
                ...
                if state.ssl_want == 'write':
                    write_list.append(sock)
                elif state.ssl_want == 'read':
                    read_list.append(sock)
                else:
                    read_list.append(sock)
                    write_list.append(sock)
            elif not state.request_complete:
