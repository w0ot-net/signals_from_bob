...
class CompatTests(unittest.TestCase):
    ...
    def test_array_frombytes_rejects_non_bytes_like(self):
        data = array.array('B')
        self.assertRaises(TypeError, array_frombytes, data, object())
    ...
    def test_to_bytes_accepts_array_py3(self):
        if PY2:
            return
        data = array.array('B', [1, 2, 3])
        self.assertEqual(to_bytes(data), b'\x01\x02\x03')
    ...
    def test_to_bytes_rejects_non_byte_itemsize_py3(self):
        if PY2:
            return
        data = array.array('H', [1, 2])
    ...
    def test_require_bytes_like_or_bytearray_rejects_memoryview_py2(self):
        if not PY2:
            return
    ...
    def test_buffer_view_memoryview_itemsize_py3(self):
        if PY2:
            return
        data = array.array('H', [1, 2, 3])
        view = memoryview(data)
        self.assertRaises(TypeError, buffer_view, view)
    ...
    def test_buffer_view_accepts_buffer_py2(self):
        if not PY2:
            return
        data = buffer(b'abcdef')
        view = buffer_view(data, length=4)
        self.assertEqual(str(view), b'abcd')
        self.assertEqual(len(view), 4)
    ...
    def test_buffer_view_accepts_memoryview_zero_length_py2(self):
        if not PY2:
            return
        data = memoryview(b'abcdef')
        view = buffer_view(data, length=0)
        self.assertEqual(str(view), b'')
    ...
    def test_buffer_view_memoryview_default_length_py3(self):
        if PY2:
            return
        data = b'abcdef'
        view = buffer_view(memoryview(data))
        self.assertEqual(view.tobytes(), data)
        self.assertEqual(len(view), len(data))

    def test_buffer_view_memoryview_zero_length_py3(self):
        if PY2:
            return
        data = b'abcdef'
        view = buffer_view(memoryview(data), length=0)
    ...
    def test_to_native_str_text_non_ascii(self):
        text = u'\u2603'
        result = to_native_str(text)
        if PY2:
            self.assertIsInstance(result, bytes)
            self.assertEqual(result, b'\xe2\x98\x83')
        else:
            self.assertIsInstance(result, str)
